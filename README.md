...
## 시간 부족으로 구현하지 못한 부분과 그 이유

> 과제 필수 기능(3.1~3.4)과 평가 4개 항목은 모두 구현했습니다. 아래는 핵심 범위를 넘어선 부분으로, 우선순위·트레이드오프상 보류했습니다.

- **프론트엔드 테스트 코드**: 평가 비중이 큰 백엔드 핵심 로직(인증·권한·도메인)을 통합/단위 테스트 58개로 우선 확보하고, 프론트는 타입체크·프로덕션 빌드 검증으로 갈음했습니다. E2E/컴포넌트 테스트는 시간상 후순위로 두었습니다.
- **`/api/auth/me` (사용자 정보 조회)**: 토큰 재발급 응답에 user를 포함하지 않아, 새로고침 직후 헤더의 이메일이 잠시 비어 보일 수 있습니다(인증·기능엔 영향 없음). 별도 엔드포인트로 해소 가능하나 영향이 작아 보류했습니다.
- **Refresh 토큰 무효화의 영속화**: 로그아웃을 인메모리 화이트리스트로 처리해 서버 재기동 시 초기화됩니다. H2 인메모리 데모 범위에 맞춘 선택이며, 운영에선 Redis 등 외부 저장소가 필요합니다.
